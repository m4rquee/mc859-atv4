...
#
	...
	print GPL_LIN_FILE <<EOF;
		...
		plot "$emp_lin_filename" t "empirical" w points, "$the_lin_filename" t "estimated" with lines, "$up_lin_filename" t "+1 std dev range" with lines, "$lo_lin_filename" t "-1 std dev range" with lines 
		quit
EOF
...
#
	...
	system("gnuplot -p $gpl_lin_filename") == 0 ||
...
#
	...
	print GPL_EXP_FILE <<EOF;
		...
		plot "$emp_exp_filename" t "empirical" w points, "$the_exp_filename" t "theoretical" with lines
		quit
EOF
...
#
	open (PS_EXP_FILE,">$ps_exp_filename") || 
                 die "Cannot open file: $ps_exp_filename \n";
	system("gnuplot -p $gpl_exp_filename") == 0 ||
                     die "gnuplot (needed for plotting) not found \n";
