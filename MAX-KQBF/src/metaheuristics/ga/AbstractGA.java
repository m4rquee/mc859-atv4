...
public abstract class AbstractGA<G extends Number, F> {
    ...
    public Solution<F> solve() {
        ...
        Chromosome popBestChromosome = bestChromosome;
        for (g = 1; g <= generations; g++) {
            ...
            population = selectPopulation(mutants, popBestChromosome);
            popBestChromosome = getBestChromosome(population);
            ...
            if (popBestChromosome.fitness > bestChromosome.fitness) {
                ...
                if (verbose)
                    System.out.println("(Gen. " + g + ") BestSol = " + -bestSol.cost);
            }
     ...
     * it with the best chromosome of the last generation.
     *
     * @param offsprings The offsprings generated by {@link #crossover}.
     * @return The updated population for the next generation.
     */
    protected Population selectPopulation(Population offsprings, Chromosome lastBestChromosome) {
        Chromosome worse = getWorseChromosome(offsprings);
        if (worse.fitness < lastBestChromosome.fitness) {
            offsprings.remove(worse);
            offsprings.add(lastBestChromosome);
        }
        return offsprings;
    }
}
